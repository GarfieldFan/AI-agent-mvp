...
### Inventory, revenue report, document-to-structured-data, variable-price products (`backend/apis/products.py`, `backend/cart.py`)

Four features built together 2026-09-10, mapping to four things an
owner should be able to say to the chatbot/owner-agent: "how's stock
today," "what was yesterday's revenue," "here's a PDF of new products,
add them," "here's a purchase order, adjust inventory." Full build
narrative — including two real bugs found and fixed via live
testing — lives in `HISTORY.md`'s 2026-09-10 "Inventory, revenue
report, document-to-structured-data, variable-price products" entry;
this section states current-state facts only.

- **Revenue report** — `generate_report`'s `report_type` is
  `"chat-volume" | "revenue"` (one shared per-day `ReportDayPoint`
  shape, `session_count`/`message_count` vs `order_count`/
  `revenue_total`). Revenue is computed from `Order.payment_status ==
  "paid"` rows only (never the owner-agent-settable free-text `status`
  field), grouped by `Order.created_at`'s date. `ReportPanel` has a
  report-type `Select` above the date picker.
- **Inventory — a two-part split, not one shared table.**
  `Product.stock_quantity`/`low_stock_threshold` (both nullable) is
  external/sellable stock — `null` means untracked, `available` alone
  still gates orderability. Once set, `checkout_cart` gates a positive
  quantity against it, and `cart.decrement_stock_and_notify(db, order)`
  runs at the moment an order is actually confirmed paid (both the
  test-provider path and the Stripe webhook's completed-session
  handler), firing `notify_owner` at/below the configured low-stock
  threshold. `StockItem` (new table: `name`/`quantity`/`unit`/
  `low_stock_threshold`) is internal raw-material inventory (coffee
  beans, milk, eggs) — deliberately never auto-linked to
  `Product.stock_quantity`, since no recipe/bill-of-materials system
  exists (a confirmed scope cut, not an oversight). `StockItemPanel`
  is a plain, much simpler CRUD panel than `ProductPanel` (no image/
  tags/price/availability), paginated the same way every other admin
  list here is, rendered directly below `ProductPanel`.
- **Variable-price ("pay what you want") products** —
  `Product.variable_price: bool`, reusing the already-existing
  `OrderItem.unit_price_snapshot` field to hold whatever amount the
  customer states rather than inventing a new column.
  `cart.apply_order_delta`'s `unit_price` parameter is strictly gated
  (`product.variable_price and unit_price is not None and unit_price >
  0`), so it can never override a normal product's real price. Settable
  via `POST /api/cart/add` (public, storefront), the chat order-
  extraction pipeline, and — closing a real gap found during live
  verification — the storefront `ProductCard`/`ProductDetail` "Add to
  cart" flow itself: `PublicProductSummary` now exposes `variable_price`
  (never `stock_quantity`/`low_stock_threshold`, which stay admin-only,
  same "not customer-facing data" posture as `available` already had),
  and both components render a "Your amount ($)" input when set.
- **Document-to-structured-data — one shared extraction primitive, two
  ...
  via `resolve_media_local_path` and parses it with
  `ingest.parse_document` — never persisted as a RAG `Document` (a
  supplier catalog/purchase order has no business in the public
  chatbot's own retrieval). `propose_products_from_document`/
  `propose_stock_from_document` (owner-agent tools) draft a batch of
  products/stock adjustments the same propose-then-owner-applies way
  `propose_products` already does, reviewed in `OwnerAgentPanel`'s
  proposal review cards; `existing_id`/`existing_quantity` on the stock
  side is resolved against a case-insensitive exact-name match so a
  reviewer sees "add to existing 3kg" vs. "new ingredient."
- **`llm_json.py` gained `extract_json_array`/`parse_lenient_json_array`**
  — an array-shaped sibling of the pre-existing, object-shaped
  `parse_lenient_json`, used by the two propose-from-document endpoints
  above (see `HISTORY.md` for why the object-shaped version silently
  discarded every correct array response). `parse_lenient_json` itself
  is untouched, still used by every existing dict-shaped caller.
- **owner-agent tool count**: +3 this round — `propose_products_from_document`,
...
### "Owner talks, AI operates" — closing tool-coverage gaps + orchestration (`owner-agent/`)

Added 2026-09-10, off a design discussion with the user: an earlier-
floated owner-agent onboarding wizard with tooltip/spotlight UI was
explicitly dropped, not deferred — if the AI performs the operation
itself, there's nothing left for a tooltip to point at; a plain chat
reply already tells the owner what happened, more directly than an
arrow could. This reframed the direction into auditing whether
owner-agent's tool-calling loop actually covers what an owner would
plausibly ask for in a real conversation. Full narrative — including a
real timeout bug found and fixed via live testing, and the full text of
the live orchestration verification — lives in `HISTORY.md`'s
2026-09-10 "Owner talks, AI operates" entry; this section states
current-state facts only.

- **`search_knowledge`** (`POST /agent/knowledge/search`) — a thin
  wrapper around `retrieval.py`'s existing `retrieve()`, the same
  function `/api/chat` already uses; owner-agent previously had no way
  to query the RAG knowledge base at all, only structured data via its
  `list_*` tools.
- **`create_document`** (`POST /agent/documents/create-from-text`) —
  saves composed text directly as a knowledge-base document, no
  upload/base64 needed (same "don't ask a text model to emit a binary
  encoding" reasoning as `generate_landing_page_from_url`).
  `apis/documents.py`'s `ingest_document` was refactored to share a new
  `_ingest_content()` helper with this route rather than duplicating
  the parse/chunk/embed/store pipeline.
- **`edit_page_field`** (`POST /agent/pages/{slug}/ai-edit`) — a
  targeted single-instruction edit to an already-saved page, applied
  immediately as a new `PageVersion` (a page edit is one click to
  undo via the dashboard's Page manager, so no propose-then-review step
  is needed — same "cheap to reverse, apply immediately" reasoning
  `set_order_status_options`/`set_shipping_allowed_regions` already
  use). Deliberately not the same mechanism as `ai_fill_content` (which
  needs a live CTE browser session and a client-computed field
  manifest) — this endpoint is fully self-contained. Sends the page's
  current sections as context, but the model only ever echoes back the
  `index` + full object of whichever section(s) it actually changed —
  a targeted diff, not a full-page round trip (see `HISTORY.md` for the
  real timeout bug this design replaced). Every named index is
  ...
  `_coerce_sections` (the same validator `generate_landing_page`
  already relies on) runs over the result.
- **`suggest_business_profile`** — registered as an owner-agent tool
  onto the already-existing `POST /agent/business-profile/suggest`, no
  backend change needed. Same propose-then-owner-applies posture as
  `propose_intent_schema`/`propose_products`. `OwnerAgentPanel` gained
  a matching editable review card (name/type/description/email/phone/
  address, Apply/Discard) — the same pattern the schema/product/stock
  review cards already established.
- **Turn/time budget raised** (`owner-agent/agent_loop.py`,
  `MAX_ITERATIONS` 6→12, `RUN_TIMEOUT_SECONDS` 300→1200) — a real
  multi-step command ("set up my whole site") can legitimately chain
  several tool calls, and the old 300s ceiling was tight enough that
  one slow `generate_landing_page` call alone (which separately budgets
  up to 620s) could exceed the whole run's budget and get it cut off on
  the very next turn-boundary check.
- **Orchestration guidance added to `SYSTEM_PROMPT_TEMPLATE`** — when
  the owner describes a new business or asks for their site to be set
  up, chain `list_intent_schemas` → `propose_intent_schema` →
  `suggest_business_profile` → (if material was given)
  `generate_landing_page`/`propose_products_from_document`/
  `propose_stock_from_document` in one run rather than stopping after
  one step, then explain plainly what's ready to review-and-Apply
  versus what already took effect. Explicitly told never to fabricate
  business-specific facts not given or found via `search_knowledge`/
  ingested documents. Verified live end-to-end: a real "I want to sell
  watches" command correctly checked for an existing schema first,
  proposed a genuinely sensible `watch_inquiry` schema it wasn't
  hand-fed, automatically chained into `suggest_business_profile`
  without being re-asked, and honestly reported an empty profile draft
  (0 documents ingested) rather than inventing facts — full transcript
  in `HISTORY.md`.
- **owner-agent tool count is now 29** — `search_knowledge`,
  `edit_page_field`, `create_document`, `suggest_business_profile` are
  the four new tools this round.
- **Inline chart rendering for `generate_report`** — settled a separate
  design question (chat-inline vs. a dedicated dashboard section) as
  BOTH, chat as the primary path, `ReportPanel` staying as a secondary,
  optional browsing entry point. The recharts rendering was extracted
  into a shared `frontend/src/components/common/report-chart.tsx`, used
  by both `ReportPanel` and `OwnerAgentPanel`'s step trace — a
  successful `generate_report` step renders the real chart inline in
  the conversation instead of a JSON blob, reusing recharts (already a
  dependency) rather than a second charting library.
