...
| `product-search-results.tsx` | (2026-08-19) `/search`'s content — plain Server Component (no fetching of its own; `/search/page.tsx` does the fetch and passes results down), same `ProductCard` grid `ProductListBlock` uses, no result cap/pagination (see the root `AGENTS.md`'s "known gap" note if you're looking at this after a catalog grows past ~20 items). |
| `cart-page.tsx` | (2026-08-20) `/cart`'s content — lists the session-scoped active `Order`'s items with quantity +/- and Remove (both resolve to `lib/cart.ts`'s `updateCartItem`, a signed `quantity_delta`), running total, "Proceed to checkout" link. Empty-cart state links to `/search`. |
...
| `ProductListBlock` | (2026-08-19, gained a real `Editable` wrapper 2026-08-20 — see below) A grid of products, client-fetched via `lib/products.ts`'s `listPublicProducts`. `category?: string\|null` filters to one category; `product_ids?: number[]\|null` (2026-08-20) is a more specific, ordered allow-list that takes priority over `category` when both are set. The one block type (alongside `ProductCardBlock`) that fetches its own data — see its own docstring for why that forces client-side fetching. |
...
| `owner-agent.ts` | `runOwnerAgentCommand(command)` → `{final_answer, stopped_reason, steps}`. Calls the separate `owner-agent` service directly (`NEXT_PUBLIC_OWNER_AGENT_URL`), not `lib/api.ts`'s backend `apiFetch` — different origin, different container. `listOwnerAgentRuns(limit?)` (2026-08-19) is the opposite: goes through backend's own `apiFetch` (`GET /api/agent/owner-agent/runs`), since that's backend's table, not owner-agent's own state. |
