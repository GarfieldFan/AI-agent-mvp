...
| `agent-console-section.tsx` | `/dashboard`'s role-gated section. `user`/logged-out sees a locked `EmptyState`; admin/owner sees every real capability grouped into a collapsible `Accordion` (2026-08-19, replacing a flat 8-panel stack that had grown hard to navigate — see `ui/accordion.tsx`): **AI & knowledge base** (`ModelSettingsPanel`, `DocumentManager`), **Content generation** (`GeoPagePanel`, `PosterGeneratorPanel`, `PageGeneratorPanel` + `PageManager`), **CRM & reporting** (`CrmPanel`, `ReportPanel`), **Owner agent** (`OwnerAgentPanel`, owner-only — see below, its own `AccordionItem` only rendered when `canViewOwnerAgent`). `multiple` open at once allowed; starts fully collapsed. No stub-card grid — every `backend/apis/agent.py` route is real. |
