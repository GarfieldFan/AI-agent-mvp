...
export function submitContactForm(input: { name: string; email: string; message: string; turnstileToken?: string | null }) {
  const { turnstileToken, ...rest } = input;
  return apiFetch<{ ok: boolean }>("/api/contact", {
    method: "POST",
    body: { ...rest, turnstile_token: turnstileToken ?? null },
  });
}
